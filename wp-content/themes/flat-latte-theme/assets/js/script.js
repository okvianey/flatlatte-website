(function() {
  ...
  /**
    ...
    **/
  document.addEventListener('DOMContentLoaded', function () {
    if (!window.location.pathname.includes('/panel-socio')) {
          return; // Salir si no es la página correcta
    }

    const paquete = document.getElementById('paquete');
    const extras = [document.getElementById('redaccion'), document.getElementById('blog'), document.getElementById('autoadministrable'), document.getElementById('chat'), document.getElementById('reservaciones')];
    ...
    function calcular() {
      const base = parseFloat(paquete.value);
      let extrasTotal = 0;
      extras.forEach(e => {
      ...
      const venta = Math.round(total * (1 + porc / 100));
      const utilidad = venta - total;

      costoTotal.textContent = total;
      precioFinal.textContent = venta;
      ganancia.textContent = utilidad;
    }

    // Escuchar cambios
    [paquete, porcentaje, ...extras].forEach(input => {
